...
DATOS SEGMENT
VECTOR1 DB 1,2,2,3 ;; Vector a comprobar va a ser repetido
VECTOR2 DB 1,2,5,4 ;; Vector a comprobar va a tener numeros diferentes al 1-4
VECTOR3 DB 1,2,3,4 ;; Vector a comprobar, va a estar bien
CONVERSION DB ?
ERROR1 DB  " -> No valido. Repeticion.",13,10,'$'
ERROR2 DB "-> No valido. Numeros diferentes 1-4",13,10,'$'
CORRECTO DB "-> Correcto.",13,10,'$'
...
PROCESO:
	...
	COMIENZO:;;COMIENZO DEL BUCLE		
		MOV AH, DS:VECTOR1[DI];; COMO ES SOLO 1B COGEMOS LA PARTE ALTA DEL CODIGO
...
DUPLICADOS PROC 
	INI:
		MOV SI, DI
		SUB SI, 4
		MOV AH, DS:VECTOR1[SI];; COMO ES SOLO 1B COGEMOS LA PARTE ALTA DEL CODIGO
		CMP AH, 4			  ;; COMPARAMOS SI ES MAYOR QUE 4
		JG ERRGT5  
		CMP AH, 1			  ;; COMPROBAMOS SI ES MENOR QUE 1
		JL ERRGT5
		ADD SI, 1
		MOV AL, DS:VECTOR1[SI];;CARGAMOS EL SIGUIENTE NUMERO
		...
		JE ERR1
		ADD SI, 1
		MOV BH, DS:VECTOR1[SI];;CARGAMOS EL SIGUIENTE NUMERO
		...
		JE ERR1
		ADD SI, 1
		MOV BL, DS:VECTOR1[SI];;CARGAMOS EL SIGUIENTE NUMERO
